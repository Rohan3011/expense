...
import { HiOutlineChevronUp, HiOutlineChevronDown, HiOutlineChevronRight } from 'react-icons/hi';


const Card = ({ title, value, amount, increase, icon, color, link }) => {
    return (
        <div className='flex flex-col gap-2 w-full h-[150px] p-4 shadow rounded-lg card-animation'>
    ...
    return (
        <div className='flex items-center'>
            <Link passHref href={link}>
                <a className='text-slate-600 border-b border-white hover:border-slate-500 text-xs'>see details</a>
            </Link>
            <Link passHref href={link}>
